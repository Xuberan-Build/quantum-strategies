...
Source: A.E. Waite, "The Real History of the Rosicrucians" (1887) — archive.org
        Chapter IV (lines ~3606–4093) contains full English translation.
Running headers ("CONFESSION OF ROSICRUCIAN FRATERNITY. NN") stripped as noise.
"""
import re
import requests
...
NOISE_PAT = re.compile(
    r"^CONFESSION\s+OF\s+ROSICRUCIAN\s+FRATERNITY\.\s*\d+\s*$"
    r"|^HISTORY\s+OF\s+THE\s+ROSICRUCIANS\.\s*\d+\s*$"
    r"|^\d{1,3}\s*$|^-\s*\d+\s*-$",
    re.IGNORECASE,
)
START_PAT = re.compile(r"^Confessio Fraternitatis R\.\s*C\.")
END_PAT   = re.compile(r"^CHAPTER V\.")

THEMES = [
    "rosicrucian", "confessio_fraternitatis", "rosicrucian_manifesto",
...
class ConfessioFraternitatisIngester(BaseIngester):
    tradition = "rosicrucianism"
    text_name = "confessio_fraternitatis"
    display_name = "Confessio Fraternitatis"
    ...
    def get_chunks(self) -> list[dict]:
        ...
        print(f"    Confessio block: {len(block)} chars")

        all_chunks = []
        for part in split_long_text(block):
            labeled = (
                "Confessio Fraternitatis — Rosicrucian Manifesto (1615)\n"
            ...
            all_chunks.append({
                ...
                "metadata": {"manifesto": "confessio_fraternitatis", "year": 1615},
            })
        return all_chunks
