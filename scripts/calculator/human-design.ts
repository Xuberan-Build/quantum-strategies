...
/**
 * 64 gates ordered around the Rave Mandala starting at 302° tropical
 * (2°00′ Aquarius). Each gate spans 5.625°. Source: Jovian Archive / Ra Uru Hu.
 *
 * Gate 41 begins at 302.0°
 * Gate 19 begins at 307.625°
 * Gate 13 begins at 313.25°
...
function norm360(lng: number): number {
...
function computeDefinedChannelsAndCenters(definedGates: Set<number>): {
  ...
  const definedChannelKeys: string[] = [];
...
/**
 * A channel key (low-high) is in definedChannelKeys?
 */
function isChannelDefined(
  ...
  const key = a < b ? `${a}-${b}` : `${b}-${a}`;
  return definedChannelKeys.includes(key);
}

/**
 * Determine whether a motor center is connected to the Throat center
 * via one or more defined channels, possibly through intermediate defined centers.
 *
 * Uses BFS over the bodygraph adjacency (defined channels only).
 *
 * Motors: Sacral, Will, Solar Plexus, Root.
 */
function isMotorConnectedToThroat(
  motorCenter: HDCenter,
  definedChannelKeys: string[],
  definedCenters: Set<HDCenter>,
): boolean {
  if (!definedCenters.has(motorCenter) || !definedCenters.has('Throat')) {
    return false;
  }

  // Build adjacency from defined channels
  const adj = new Map<HDCenter, Set<HDCenter>>();
  const addEdge = (a: HDCenter, b: HDCenter) => {
  ...
  for (const [a, b] of CHANNELS) {
    const key = a < b ? `${a}-${b}` : `${b}-${a}`;
    if (definedChannelKeys.includes(key)) {
  ...
  // BFS from motorCenter to Throat
  const visited = new Set<HDCenter>();
  const queue: HDCenter[] = [motorCenter];
  visited.add(motorCenter);
...
function deriveType(
  ...
  const motorToThroat = motors.some(m =>
    isMotorConnectedToThroat(m, definedChannelKeys, definedCenters),
  );
  ...
  // Sacral not defined, no motor to Throat
  return 'Projector';
}
...
function deriveAuthority(
  ...
  // Self-Projected: G center defined and connected to Throat
  if (
    definedCenters.has('G') &&
    isMotorConnectedToThroat('G' as HDCenter, definedChannelKeys, definedCenters)
  ) {
    return 'Self-Projected';
  }

  // Mental / Environmental: Head or Ajna defined (no lower motor)
  if (definedCenters.has('Head') || definedCenters.has('Ajna')) {
    return 'Mental';
  }

  // Lunar: Reflector (0 defined centers)
  if (definedCenters.size === 0) {
    return 'Lunar';
  }
...
function deriveDefinition(
  ...
  for (const [a, b] of CHANNELS) {
    const key = a < b ? `${a}-${b}` : `${b}-${a}`;
    if (!definedChannelKeys.includes(key)) continue;
    const ca = GATE_CENTERS[a] as HDCenter;
    const cb = GATE_CENTERS[b] as HDCenter;
...
export function buildHumanDesignChart(
  ...
  // 4. Derive all HD properties
  const type       = deriveType(definedCenters, definedChannelKeys);
  const authority  = deriveAuthority(definedCenters, definedChannelKeys);
  const profile    = deriveProfile(conscious, unconscious);
  const definition = deriveDefinition(definedCenters, definedChannelKeys);

  // 5. Split defined/undefined centers
  ...
  return {
    ...
    definedChannels:  definedChannelKeys,
  };
}
