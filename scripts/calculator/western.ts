...
function placidusHybridCusps(ascendant: number, midheaven: number): number[] {
  const asc = mod360(ascendant);
  const mc  = mod360(midheaven);
  ...
  const h11 = mod360(mc  + angularArcSigned(mc,  asc) / 3);
  const h12 = mod360(mc  + angularArcSigned(mc,  asc) * 2 / 3);
  const h2  = mod360(asc + angularArcSigned(asc, ic)  / 3);
  const h3  = mod360(asc + angularArcSigned(asc, ic)  * 2 / 3);
  const h5  = mod360(ic  + angularArcSigned(ic,  dsc) / 3);
  const h6  = mod360(ic  + angularArcSigned(ic,  dsc) * 2 / 3);
  const h8  = mod360(dsc + angularArcSigned(dsc, mc)  / 3);
  const h9  = mod360(dsc + angularArcSigned(dsc, mc)  * 2 / 3);
...
function isApplying(
  ...
  const rawDiff = mod360(lng2 - lng1);
  ...
  const distA = mod360(rawDiff - targetA); // how far rawDiff is from targetA going forward
  const distB = mod360(rawDiff - targetB);
  ...
  const needToGainOnP2 = mod360(nearestTarget - rawDiff) < 180;
