...
function buildDashas(moonSiderealLng: number, birthUtc: Date): DashaPeriod[] {
  const n = norm360(moonSiderealLng);
  ...
  // 4. Build one full 120-year Vimshottari cycle (9 periods)
  const periods: DashaPeriod[] = [];
  let cursor = birthUtc;

  for (let i = 0; periods.length < 9; i++) {
...
export function buildVedicChart(
  ...
  const lagnaSignIndex = Math.floor(norm360(siderealAsc) / 30);
  ...
  for (const planetName of PLANET_NAMES) {
    ...
    const planetSignIndex = Math.floor(norm360(siderealLng) / 30);
